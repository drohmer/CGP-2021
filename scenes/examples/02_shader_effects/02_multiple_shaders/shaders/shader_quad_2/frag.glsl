...
void main()
{
	vec3 modified_color = fragment.color;
	if( length(fragment.position.st-vec2(0.25,0.5))<0.1 ){
		discard;
	}
	if( length(fragment.position.st-vec2(0.75,0.5))<0.1 ){
		discard;
	}
	if( length(fragment.position.st-vec2(0.5,0.25))<0.1 ){
		discard;
	}
	if( length(fragment.position.st-vec2(0.5,0.75))<0.1 ){
		discard;
	}
	if( length(fragment.position.st-vec2(0.5,0.5))>0.4 ){
		modified_color = vec3(0,1,0.5);
	}
	...
	vec3 color_object  = modified_color * color * color_image_texture.rgb;
