...
void implicit_surface_structure::update_marching_cube(float isovalue)
{
	...
	if (position.size() > previous_size) {
		...
		drawable_param.shape.send_data_to_GPU(position, normal);
	}
	else {
		// Otherwise simply update the new relevant values re-using the allocated buffers
		drawable_param.shape.update_position(position, number_of_vertex);
		drawable_param.shape.update_normal(normal, number_of_vertex);
	}
	drawable_param.shape.number_elements = number_of_vertex;
}
...
void implicit_surface_structure::update_field(field_function_structure const& field_function, float isovalue)
{
	...
	drawable_param.domain_box = spatial_domain_grid_drawable(domain);
}
